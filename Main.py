...
def newEmployee():
    while True:
        ...
        while True:
            ...
            elif not set(empPostal[0]).issubset(empPostalCharAllowed) or not set(empPostal[2]).issubset(
                    empPostalCharAllowed) or not set(empPostal[4]).issubset(empPostalCharAllowed):
                print("1Enter a valid Postal Code.")
            elif not set(empPostal[1]).issubset(empPostalNumAllowed) or not set(empPostal[3]).issubset(
                    empPostalNumAllowed) or not set(empPostal[5]).issubset(empPostalNumAllowed):
...
def customReport():
    ...
    while True:
        ...
        if driver_number in driver_info:
            ...
            print()
            print(
                "Date          Transaction ID          Payment Description          Subtotal          HST          Total")
            print("-" * 103)
